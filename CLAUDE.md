...
- **Scrapers:** Python + Selenium (TDN sales, Equineline), plain `requests` (TDN sire list); GitHub Actions — full scrape daily at 12:30 AM UTC. Current-year sire rankings run from the Fly.io parser every 30 min (`sire-rankings.yml` remains as a backstop)
...
- **Current-year rankings run from the Fly.io parser**, not GitHub Actions: `main.py`'s `run_rankings()` calls `scrape_all_stallions(do_sales=False, do_equineline=False, current_year_only=True)` every 30 min (`--rankings-interval`, 0 disables), plus once at startup so a redeploy refreshes immediately. It swallows every exception — `schedule.run_pending()` re-raises a failed job into the polling loop's outer handler, which sleeps 30s and would stall the 1-minute email poll.
- Two workflows remain: `daily-scraper.yml` (12:30 AM UTC — sales + Equineline + historical ranking backfill, still Selenium) and `sire-rankings.yml` (hourly — current-year rankings, pure HTTP). The latter is now a backstop; writes are idempotent upserts, so it and the parser can both run without conflict.
- Rankings are upserted on `(stallion_id, year, list_type)`, so repeat runs are idempotent and the frequent job is safe.
- **Why the parser owns this cadence:** TDN posts the prior day's results at an unpublished time (observed absent at 06:00 UTC and present by 20:47 UTC; on 2026-09-10 the freshman list changed between 09:43 and 13:35 UTC), so a single overnight scrape left the StatsBar a day behind the near-real-time results feed. Frequent polling removes the need to guess — but GitHub's cron cannot deliver it. Measured on this repo over 12 scheduled runs (Sep 8–10, 2026), an **hourly** cron fired every 3.9h on average (min 2.4h, max 4.9h); 0 of 11 gaps were under 1.5h, and the `*/15` pollers drift the same way. `daily-scraper.yml`'s `30 0 * * *` has been firing at ~05:00 UTC. That is why the rankings scrape moved to the always-on parser. **Scheduled workflows only run from the default branch**: a workflow file on a feature branch never fires.
- Historical years are closed and cannot change — `--current-year-only` skips them so the hourly job makes ~10 requests, not ~35.
