  ...
  return (
    <header className="sticky top-0 z-10 text-white px-3 sm:px-6 pb-0.5 sm:py-3" style={{ backgroundColor: 'var(--org-primary)', paddingTop: 'max(0.125rem, env(safe-area-inset-top))' }}>
      <div className="max-w-5xl mx-auto">
        {/* Mobile: Two rows */}
        {/* Desktop: Single row */}
        <div className="flex sm:flex-row sm:items-baseline sm:justify-between sm:gap-3">
          {/* Single row: left group (arrow + name) + right group (nav) */}
          <div className="flex items-baseline gap-1 sm:gap-3 min-w-0">
            <Link
              href="/dashboard"
          ...
          <div className="flex sm:hidden items-baseline gap-1 shrink-0" style={{ color: 'var(--org-secondary)' }}>
            {hasBookings && (
              <Link
                ...
                className="text-xs font-medium uppercase tracking-wide opacity-80 hover:opacity-100 transition-opacity px-0.5"
              >
                Bookings
              </Link>
            )}
            {onExportPDF && (
              <button
                ...
                className="p-0.5 inline-flex items-center justify-center hover:text-white transition-colors disabled:opacity-50"
                title="Export PDF"
              >
                {isExporting ? (
